...
const DATABASE_DIRECTORY = path.join(process.cwd(), "database", "meditations_db");
...
async function createAndSaveEmbeddings() {
    ...
    |================ LOADING JSON's =================| 
    +------------------------------------------------*/
    const jsonDirectoryPath = process.env.JSON_MEDITATIONS_PATH;
    if (!jsonDirectoryPath) {
        return "Erro: JSON_MEDITATIONS_PATH não definido";
    }
